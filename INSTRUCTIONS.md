...
The No. 1 thing about this app is to be patient. Each landing page load, data save, and change of scope on the data tab triggers a flow that reads or writes data from a SPO site. There can be several seconds between a click and a visible change. That's the tradeoff for being able to host this page publicly while storing all of the data securely.
...
## The Actions
### 1 - Hours
Select the date then enter hours in one of the six categories. The app present an alert if more than eight hours are entered. That's okay if OT is included.
